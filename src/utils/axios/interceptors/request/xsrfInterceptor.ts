...
import { AxiosInstance, Options, XsrfConfig } from 'axios';
...
export default (axios: AxiosInstance) => {
  axios.interceptors.request.use(
    async function(config) {
      ...
      const { url, xsrfHeaderName, getToken, timeout = 0 } = xsrfConfig as XsrfConfig;
      ...
      if (!getToken || !url) {
        return Promise.reject(
          new Error('Neithor xsrf token found nor url for getting new xsrf token provide.'),
        );
      }
      // get xsrf token
